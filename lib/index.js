...
function currentForecastHtml1(data) {
  var currently1 = `<div class='icon-c1'> ${data.icon} </div>
                    ...
                    <div class='time-c1'> ${data.time} </div>`
...
function hourlyForecastHtml(data) {
  ...
  for (i = 0; i < data.length; i++) {
    var hour = `<div class='hour-h'> ${data[i].time} </div>
                <div class='icon-h'> ${data[i].icon} </div>
                <div class='temp-h'> ${data[i].temperature}˚ </div></br>`
    hourly.push(hour)
    var joinedHourly = hourly.join(',').replace(/,/g, ' ').split();
  };
  hourlyForecast.innerHTML = joinedHourly;
};
...
function dailyForecastHtml(data) {
  ...
  var i;
  for (i = 0; i < data.length; i++) {
    var day = `<div class='day-d'> ${data[i].time} </div>
               ...
               <div class='low-d'> Low ${data[i].low}˚ </div></br>`
    daily.push(day)
    var joinedDaily = daily.join(',').replace(/,/g, ' ').split();
  };
  dailyForecast.innerHTML = joinedDaily;
};
...
function favoritesHtml(data) {
  ...
  favorites.innerHTML = joinedFaves;
};
